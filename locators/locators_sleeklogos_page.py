...
class LocSleekLogos(LocatorsBase):
    ...
    icons_resuilt = './/*[@class="b-subcategory-wrapper"][1]' \
                    '/descendant::span[1]'
    category_list = './/*[@class="b-bar-menus-menu m-scrollable"]' \
                    '/descendant::a[%s]'
    platform_search_name = '//h1[@class="icons-set-title"]' \
                           '[contains(text(), "%s")]'
    ...
    label = './/*[@class="b-subcategory-wrapper"][1]' \
            '/descendant::*[@class="icons-set__icon-title"][1]'
    ...
    icon_in_icon_page = './/*[@class="col-md-4 m-full-width' \
                        ' b-main-icon m-main-icon"]'
    ...
    confirm_delete_icon = '//div[@class="c-btn modal__action-confirm' \
                          ' modal__action"]'
    delete_menu_collections = '''.//*[@ng-class="{'m-edit': ''' \
                              '''collectionsEdit}"]'''
    delete_collection = './/*[@ng-repeat="coll in colls.colls"][%s]' \
                        '/*[@class="b-collection-delete"]'
    confirm_delete_collection = '//*[@class="c-btn modal__action-confirm' \
                                ' modal__action"]'
    rename_collection = './/*[@ng-click="collsControl.showRenaming()"]'

    generate_html = '//*[@class="b-bar-btns m-icon m-single-btn"]/*[2]/btn'
    open_color_panel = '''.//*[@ng-include="'/template-icon.html'"][1]''' \
                       '''/descendant::*[@class="off-click-recolor"]'''
    grayscale = '//*[@class="colors"]/descendant::*[@ng-repeat="color_' \
                ' in colorsGray"][%s]' % random.randint(1, 5)
    color = '//*[@class="colors-block"][2]/*[%s]' % random.randint(1, 10)
    color_palette = '//*[@class="colors"]/descendant::' \
                    '*[@ng-class="{active: showPicker}"]'
    canvas = '//*[@class="colors"]/descendant::*/canvas'

    download_popup = '''.//*[@ng-class="{'m-single-page': pageType === 'single',''' \
                     ''' 'm-icon-state':iconState == 'icon'}"]/*[1]/span[1]'''
    right_bar = '''//*[@i8-scroll-commander="vm.scrollCommander"]'''

    small_size = '''.//*[@ng-controller="IconCtrl"]''' \
                 '''/descendant::li[@ng-show="isShowSize(size)"][1]'''
    middle_size = '''.//*[@ng-controller="IconCtrl"]''' \
                  '''/descendant::li[@ng-show="isShowSize(size)"][2]'''
    big_size = '''.//*[@ng-controller="IconCtrl"]''' \
               '''/descendant::li[@ng-show="isShowSize(size)"][3]'''
    ...
    open_icon_editor = '''.//*[@ng-mouseleave="showTooltipEffect[effect] = ''' \
                       '''false"][1]/*[@src="'/effectBtn/'+effect+'.html'"]'''
    ...
    icon_color_edit = './/*[@class="b-editor-icon-actions f-left"]' \
                      '/descendant::*[@class="b-editor-recoler' \
                      ' b-editor-recoler-icon off-click-recolor"]'
    overlay_color_edit = './/*[@class="b-editor-icon-actions f-left"]' \
                         '/descendant::*[@class="b-editor-recoler' \
                         ' off-click-recolor b-editor-recoler-overlay"]'
